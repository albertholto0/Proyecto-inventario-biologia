...
<html lang="es">
    ...
    <body>
        ...
        <div class="container mb-5">
            ...
            <div class="modal fade" id="modalDetallesPrestamo" tabindex="-1" aria-labelledby="modalDetallesPrestamoLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header bg-success text-white">
                            <h5 class="modal-title" id="modalDetallesPrestamoLabel">Detalles del Préstamo #<span id="prestamo-id-modal"></span></h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <jsp:useBean id="detalleEquipoService" class="com.unsij.services.DetallePrestamoEquipoService" scope="page"/>
                            <jsp:useBean id="detalleMaterialService" class="com.unsij.services.DetallePrestamoMaterialService" scope="page"/>

                            <div class="mb-4">
                                <h6>Equipos prestados:</h6>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                if (modalDetalles) {
                    modalDetalles.addEventListener('show.bs.modal', function (event) {
                        ...
                        var idPrestamo = button.getAttribute('data-id');
                        
                        document.getElementById('prestamo-id-modal').textContent = idPrestamo;

                        window.location.href = 'prestamos.jsp?idPrestamo=' + idPrestamo;
                    });
                }

                var urlParams = new URLSearchParams(window.location.search);
                var idPrestamoParam = urlParams.get('idPrestamo');

                if (idPrestamoParam) {
                    var modal = new bootstrap.Modal(document.getElementById('modalDetallesPrestamo'));
                    modal.show();
            ...
            <c:forEach items="${detalleEquipoService.obtenerPorPrestamo(idPrestamo)}" var="equipo">
                    document.getElementById('tabla-equipos').innerHTML +=
                            '<tr><td>${equipo.nombreEquipo}</td><td>${equipo.cantidad}</td></tr>';
            </c:forEach>

            <c:forEach items="${detalleMaterialService.obtenerPorPrestamo(idPrestamo)}" var="material">
                    document.getElementById('tabla-materiales').innerHTML +=
                            '<tr><td>${material.nombreMaterial}</td><td>${material.cantidad}</td></tr>';
            </c:forEach>

                    if (document.getElementById('tabla-equipos').innerHTML === '') {
                        document.getElementById('tabla-equipos').innerHTML =
                                '<tr><td colspan="2" class="text-muted">No hay equipos en este préstamo</td></tr>';
                    }
                    if (document.getElementById('tabla-materiales').innerHTML === '') {
                        document.getElementById('tabla-materiales').innerHTML =
                                '<tr><td colspan="2" class="text-muted">No hay materiales en este préstamo</td></tr>';
                    }
